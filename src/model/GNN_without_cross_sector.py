...
def plot_unscaled_predictions(pred, graph_data, df, ticker_index_map):
    ...
    ax.grid(True)
    plt.tight_layout()
    png_path = get_artifact_path("gnn_vs_actual_prices_hd_without_cross_sector.png")
    svg_path = get_artifact_path("gnn_vs_actual_prices_hd_without_cross_sector.svg")
...
def forecast_future_prices(model, df, granger_df, horizons=[30, 180, 365], n_samples=50):
    ...
    forecast_csv_path = get_artifact_path("gnn_forecast_confidence_final_without_cross_sector.csv")
